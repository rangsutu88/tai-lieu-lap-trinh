/*A table can have only one clustered index because the clustered index dictates 
the physical storage of the data
Primary key constraint creates clustered index automatically if clustered index
...
select * from T1
GO
IF EXISTS (SELECT name FROM THU..sysindexes 
      WHERE name = 'c1_index')
